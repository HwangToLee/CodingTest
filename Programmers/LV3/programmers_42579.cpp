...
vector<int> solution(vector<string> genres, vector<int> plays)
{
    ...
    vector<pair<string, int>> genreRank(genreCount.begin(), genreCount.end());
    sort(genreRank.begin(), genreRank.end(), [](const pair<string, int> &a, const pair<string, int> &b)
         { return a.second > b.second; });

    for (int i = 0; i < genreRank.size(); i++)
    {
        vector<pair<int, int>> temp = genreMap[genreRank[i].first];
