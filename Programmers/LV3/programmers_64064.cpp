...
{
    auto matches = [&](const string &u, const string &b)
    {
        if (u.size() != b.size())
            return false;
    ...
    vector<vector<int>> cand(banned_id.size());

    for (size_t j = 0; j < banned_id.size(); ++j)
    {
        for (size_t i = 0; i < user_id.size(); ++i)
        {
            if (matches(user_id[i], banned_id[j]))
            {
                cand[j].push_back(static_cast<int>(i));
            }
        }
    }

    unordered_set<int> seen;
    int m = static_cast<int>(banned_id.size());

    function<void(int, int)> dfs = [&](int idx, int usedMask)
    {
        if (idx == m)
        {
            seen.insert(usedMask);
            return;
        }
        for (int uidx : cand[idx])
        {
            int bit = 1 << uidx;
            if (usedMask & bit)
                continue;
